...
SF_API void
_add_native_isnumeric (Vec<Statement *> &ast)
{
  NativeFunction *nv = new NativeFunction (_native_isnumeric, { "self" });
  nv->set_self_arg (true);
  I (nv);
  ast.insert (0, static_cast<Statement *> (new VarDeclStatement (
                     static_cast<Expr *> (new VariableExpr ("''.isnumeric")),
                     static_cast<Expr *> (
