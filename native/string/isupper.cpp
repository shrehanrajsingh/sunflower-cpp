...
_native_isupper (Module *mod)
{
  ...
  for (size_t i = 0; i < val.size (); ++i)
    {
      if (!std::isupper (static_cast<unsigned char> (val[i])))
        {
          result = false;
          break;
        }
...
SF_API void
_add_native_isupper (Vec<Statement *> &ast)
{
  NativeFunction *nv = new NativeFunction (_native_isupper, { "self" });
  nv->set_self_arg (true);
  I (nv);
  ast.insert (0, static_cast<Statement *> (new VarDeclStatement (
                     static_cast<Expr *> (new VariableExpr ("''.isupper")),
                     static_cast<Expr *> (
