...
SF_API void
_add_native_isspace (Vec<Statement *> &ast)
{
  NativeFunction *nv = new NativeFunction (_native_isspace, { "self" });
  nv->set_self_arg (true);
  I (nv);
  ast.insert (0, static_cast<Statement *> (new VarDeclStatement (
                     static_cast<Expr *> (new VariableExpr ("''.isspace")),
                     static_cast<Expr *> (
