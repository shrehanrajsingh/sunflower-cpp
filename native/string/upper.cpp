  ...
  for (size_t i = 0; i < val.size (); ++i)
    {
      result_str += static_cast<char> (
          std::toupper (static_cast<unsigned char> (val[i])));
    }
...
SF_API void
_add_native_upper (Vec<Statement *> &ast)
{
  NativeFunction *nv = new NativeFunction (_native_upper, { "self" });
  nv->set_self_arg (true);
  I (nv);
  ast.insert (0, static_cast<Statement *> (new VarDeclStatement (
                     static_cast<Expr *> (new VariableExpr ("''.upper")),
                     static_cast<Expr *> (
